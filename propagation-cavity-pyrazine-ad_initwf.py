...
#-------- 
 ...
 coef_gs_t=np.zeros((tot_grid,tot_grid,n_fock),dtype=complex)
 coef_ex1_t=np.zeros((tot_grid,tot_grid,n_fock),dtype=complex)
 coef_ex2_t=np.zeros((tot_grid,tot_grid,n_fock),dtype=complex)
 
 coef_gs_t[:,:,0]=(coef_gs[:,:,0].flatten()*di_to_ad[:,0,0]+coef_ex1[:,:,0].flatten()\
                      *di_to_ad[:,0,1]+coef_ex2[:,:,0].flatten()*di_to_ad[:,0,2]).reshape(tot_grid,tot_grid) 
 coef_ex1_t[:,:,0]=(coef_gs[:,:,0].flatten()*di_to_ad[:,1,0]+coef_ex1[:,:,0].flatten()\
                      *di_to_ad[:,1,1]+coef_ex2[:,:,0].flatten()*di_to_ad[:,1,2]).reshape(tot_grid,tot_grid)
 coef_ex2_t[:,:,0]=(coef_gs[:,:,0].flatten()*di_to_ad[:,2,0]+coef_ex1[:,:,0].flatten()\
                      *di_to_ad[:,2,1]+coef_ex2[:,:,0].flatten()*di_to_ad[:,2,2]).reshape(tot_grid,tot_grid)

 coef_gs=np.copy(coef_gs_t)
 coef_ex1=np.copy(coef_ex1_t)
 coef_ex2=np.copy(coef_ex2_t)
#----------------------------------------------------------
 # Run the propagation.
