...
# Run the propagation
 ...
 GB_file=open("response_GB_NR_02.txt", "w+")
 ...
 for t in range(0,t2_step,output):
  ...
  for step in range(n_step):
   ...
   if time_fs==t1:
    ...
    ket_coef_ex2[:,:,0]=(coef*0.979).astype(complex) 
   ...
   if time_fs==t2:
    for i in range(tot_grid):
     for j in range(tot_grid):
      ket_coef_gs[i,j,:]=np.dot(dipol_di20[:,:],ket_coef_ex2[i,j,:])
      ket_coef_ex2[i,j,:]=complex(0.0,0.0)
