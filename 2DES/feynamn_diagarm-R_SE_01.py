...
# Run the propagation
 ...
 GB_file=open("response_SE_R_01.txt", "w+")
 dipol_di20=np.zeros((n_fock,n_fock))
 for i in range(n_fock):
  dipol_di20[0,0]=0.276
 ...
 for t in range(0,t2_step,output):
  ...
  for step in range(n_step):
   ...
   if time_fs==t1:
    ...
    bra_coef_ex1[:,:,0]=(coef*0.276).astype(complex) 
...
#    print((abs(ket_coef_gs[:,:,0])**2).sum())
   ...
   if time_fs==t2:
    for i in range(tot_grid):
     for j in range(tot_grid):
      ket_coef_ex1[i,j,:]=np.dot(dipol_di20[:,:],ket_coef_gs[i,j,:])
   ...
   if time_fs==t3:
    for i in range(tot_grid):
     for j in range(tot_grid):
      bra_coef_gs[i,j,:]=np.dot(dipol_di20[:,:],bra_coef_ex1[i,j,:])
      bra_coef_ex1[i,j,:]=complex(0.0,0.0)
...
#    print((abs(ket_coef_ex2[:,:,0])**2).sum())
   ...
   if (time_fs>=t3) and (step==out):
    for i in range(tot_grid):
     for j in range(tot_grid):
      temp_coef[i,j,:]=np.dot(dipol_di20[:,:],ket_coef_ex1[i,j,:])
