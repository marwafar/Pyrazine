...
#--------------------------------------------
 ...
 bra_coef_pol=np.zeros((tot_grid,tot_grid,n_fock),dtype=complex)
 ket_coef_pol=np.zeros((tot_grid,tot_grid,n_fock),dtype=complex) 
...
# Run the propagation
 ...
 GB_file=open("response_GB_R_02.txt", "w+")
 ...
 for t in range(0,t2_step,output):
  ...
  for step in range(n_step):
   ...
   if time_fs==t1:
    ...
    bra_coef_ex2[:,:,0]=(coef*0.979).astype(complex) 
    ket_coef_gs[:,:,0]=coef.astype(complex)
   ...
   if time_fs==t2:
    for i in range(tot_grid):
     for j in range(tot_grid):
      bra_coef_gs[i,j,:]=np.dot(dipol_di20[:,:],bra_coef_ex2[i,j,:])
      bra_coef_ex2[i,j,:]=complex(0.0,0.0)
...
#    print((abs(bra_coef_gs[:,:,0])**2).sum())
   ...
   if time_fs==t3:
    for i in range(tot_grid):
     for j in range(tot_grid):
      ket_coef_ex2[i,j,:]=np.dot(dipol_di20[:,:],ket_coef_gs[i,j,:])        
...
#    print((abs(ket_coef_ex2[:,:,0])**2).sum())
   ...
   if (time_fs>=t3) and (step==out):
    for i in range(tot_grid):
     for j in range(tot_grid):
      temp_coef[i,j,:]=np.dot(dipol_di20[:,:],ket_coef_ex2[i,j,:])

    correlation=response(temp_coef[:,:,0],bra_coef_gs[:,:,0])
